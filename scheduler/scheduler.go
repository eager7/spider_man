...
func (sched *myScheduler) saveReqToCache(req base.Request, code string) bool {
	...
	if httpReq == nil {
		...
		mlog.Warn.Println("Ignore the request! It's HTTP request is invalid!")
		return false
	}
	reqUrl := httpReq.URL
	if reqUrl == nil {
		//logger.Warnln("Ignore the request! It's url is is invalid!")
		mlog.Warn.Println("Ignore the request! It's url is is invalid!")
		return false
	}
	if strings.ToLower(reqUrl.Scheme) != "http" {
		//logger.Warnf("Ignore the request! It's url scheme '%s', but should be 'http'!\n", reqUrl.Scheme)
		mlog.Warn.Println("Ignore the request! It's url scheme '%s', but should be 'http'!\n", reqUrl.Scheme)
		return false
	}
	if _, ok := sched.urlMap[reqUrl.String()]; ok {
		//logger.Warnf("Ignore the request! It's url is repeated. (requestUrl=%s)\n", reqUrl)
		mlog.Warn.Println("Ignore the request! It's url is repeated. (requestUrl=%s)\n", reqUrl)
		return false
	}
	if pd, _ := getPrimaryDomain(httpReq.Host); pd != sched.primaryDomain {
		...
		mlog.Warn.Println("Ignore the request! It's host '%s' not in primary domain '%s'. (requestUrl=%s)\n",
	...
	if req.Depth() > sched.crawlDepth {
		...
		mlog.Warn.Println("Ignore the request! It's depth %d greater than %d. (requestUrl=%s)\n",
